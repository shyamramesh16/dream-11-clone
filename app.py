...
def get_sql_response(df, question):
    plot_prompt = {
            ...
            Based on the below dataframe called as df and user query write a python query to store the output as dataframe to variable df_r and plot the output data in a python graph. The below dataframe contains the required information, you need to answer the question.
            ...
            Write the python query to display the x-axis and y-axis output create plots and graphs from the dataframe and nothing else. 
...
if user_query is not None:
    ...
    st.write(get_sql_response(df = df, question = user_query))
    # st.write(response)
    try:
            exec(plot_response)
            if 'chart' in globals():
